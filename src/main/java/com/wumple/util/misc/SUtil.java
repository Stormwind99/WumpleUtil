...
public class SUtil
{
    ...
    /**
     * shrink the itemStack in itemStacks[currentItemSlot] by amount If itemStack is a contained item and used up, leave behind empty container
     * 
    ...
    /*
     * Can we grow or add count to destination itemstack?
     */
    public static boolean canAddOrGrowCount(ItemStack destination, int add)
    {
        if (!destination.isEmpty())
        {
            int count = add + destination.getCount();
            if (count <= destination.getMaxStackSize())
            {
                return true;
            }
