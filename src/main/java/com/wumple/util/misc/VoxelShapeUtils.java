...
// from https://github.com/mekanism/Mekanism/blob/1.15x/src/main/java/mekanism/common/util/VoxelShapeUtils.java

public final class VoxelShapeUtils {

    private static final Vec3d fromOrigin = new Vec3d(-0.5, -0.5, -0.5);

    protected static void logger(String message)
    {
    	
    }
    
    public static void print(double x1, double y1, double z1, double x2, double y2, double z2) {
        logger("makeCuboidShape(" + Math.min(x1, x2) + ", " + Math.min(y1, y2) + ", " + Math.min(z1, z2) + ", " +
    ...
    public static VoxelShape getSlope(float minX, float minY, float minZ, float maxX, float maxY, float maxZ, float rotationPointX, float rotationPointY,
        ...
        if (rotateAngleX == 0 && rotateAngleY == 0 && rotateAngleZ == 0) {
            ...
            //logger("Min: {}, Max: {}, MinCenter: {}, MaxCenter: {}", min.add(shiftX, shiftY, shiftZ), max.add(shiftX, shiftY, shiftZ), minCenter.add(shiftX, shiftY, shiftZ), maxCenter.add(shiftX, shiftY, shiftZ));
            return VoxelShapes.empty();
        }
